<?php
/**
 * update_estado_pago.php — VERSIÓN BLINDADA
 *
 * CAMBIOS RESPECTO AL ORIGINAL:
 *  1. La llamada a Google Calendar está dentro de un try/catch independiente.
 *     Si falla (timeout, credenciales, fechaFinal/horaFinal nulos), el script
 *     NO aborta: devuelve success:true con calendar_error:true.
 *     En el original, un error no capturado en actualizarEventoCalendar podía
 *     cortar la ejecución ANTES del echo json_encode(), enviando HTML de error
 *     al cliente en lugar de JSON. Android recibía un cuerpo no parseable,
 *     Retrofit lanzaba una excepción, el catch de Kotlin devolvía null, y el
 *     código de rollback no se ejecutaba → bug silencioso.
 *
 *  2. Protección explícita contra fechaFinal/horaFinal nulos: si son nulos,
 *     se usan fecha/horaInicio como fallback en lugar de enviar fechaEnd=false
 *     a la API de Calendar.
 *
 *  3. La columna SQL es `estadoPago` (camelCase) — confirmado en ItemEntity.kt
 *     y en el DDL SQL del dump. No se cambia; solo se documenta.
 *
 *  4. Buffer de output desactivado para evitar que errores PHP parciales
 *     contaminen el JSON.
 */
